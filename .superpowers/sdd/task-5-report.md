...
- 演示数据升级为 v4；当 v4 不存在时，仓库从真实 `deang-sour-tea:v3` 执行幂等字段级迁移，保留业务数据，只替换两个稳定 seed ID 的权威文化内容和已知内置旧图片路径。
...
- `frontend/src/tests/public/PublicRouter.spec.ts`
...
独立复核修复第一次运行 `npm test -- src/tests/data/demoRepository.spec.ts`：退出码 1。真实 v3 key 中的自定义会话丢失，自定义内容哨兵不存在，2 个迁移测试失败，证明旧实现执行了整库重建。
...
- 生产构建主包约 1,074 KB，Vite 报告超过 500 KB；Task 5 公共页面已拆为独立懒加载 chunk，但 Element Plus 等全局依赖的按需加载属于跨任务性能优化。
